...
\hypersetup{
  pdftitle={The Kalman filter and the square root Kalman filter},
  pdfauthor={Koichi (Koiti) Yano},
  colorlinks=true,
  linkcolor={blue},
  filecolor={Maroon},
  citecolor={Blue},
...
\title{The Kalman filter and the square root Kalman filter}
...
Kalman Filter is used to estimate the state vector \(x_t\) given the
observations \(y_t\).
...
where \eqn{x(t|t)} is the estimated state vector at, \#' \eqn{P(t)} is
the estimated state covariance matrix, \#' and \eqn{K(t)} is the Kalman
gain at time t.
